...
void Write_Process(struct traceline *T_line, int ec, int policy)
{
	...
	{
//		if(fp->addr != T_line->Address)
//		{
//			printf("Not equal! It is virtual address!\n");
//		}

		w_case_3++;
//		printf("FP hit![%lld]!\n", w_case_3);
		total_fp_hit++;
		fp->replica++;//excluding its first birth.

		long r_ID = atol(T_line->RequestID);
		R_Map[fp].insert(r_ID);
		...
		else
		{
			...
			W_Req_Tbl[policy][T_line->Request_index].remain.erase(T_line->fp->p->node_num);
...
void Read_by_FP(struct traceline *T_line, int policy, int cp)
{
	...
	//increment_r_access of the pblk
	global_storage[pblk_num]->count_r_access++;
	...
	//corrupt the data.
	Data_corruption(pblk_num);

//	T_line->fp->access_times++;
	...
	{
//		printf("line[%lld]\n", total_line);
		printf("line[%8lld] RID: %s [dataNum: %3lld] P_num[%5lld]--> ACCESS Node[%3d]  [Corrupted]\n", total_line, T_line->RequestID, datapblk_num, pblk_num, CurNode);
	}

//	Check_F_cache(cp, T_line);
	if(Access_F_Cache(cp, pblk_num))//F_cache hit
	{
		r_case_2++;
		F_cache_hit++;
...
//			printf("line{%8lld] RID: %s [dataNum: %3lld] P_num[%5lld]--> ACCESS Node[%3d]  [NORMAL]\n", total_line, T_line->RequestID, datapblk_num, pblk_num, CurNode);

			Cluster[CurNode]->total_access_count++;
			Cluster[CurNode]->read_access++;

//			printf("totalline: %lld ACCESS Node[%3d]: %5lld --> ", total_line, CurNode, Cluster[CurNode]->queques[queue]);
...
//			//caching data
//			if(cp >= 3)
//				Check_F_Cache(cp, pblk_num);
		}
		else// corrupted block
		{
//			printf("line[%8lld] RID: %s [dataNum: %3lld] P_num[%5lld]--> ACCESS Node[%3d]  [Corrupted]\n", total_line, T_line->RequestID, datapblk_num, pblk_num, CurNode);
			long long t = -1;
			//caching data
			if(Access_F_Cache(cp, pblk_num))
			{
				if (sys_last_busy_time > Cluster[CurNode]->queques[queue])
				{
					Cluster[CurNode]->queques[queue] = sys_last_busy_time;
				}
				else
				{
					sys_last_busy_time = Cluster[CurNode]->queques[queue];
				}

				t = sys_last_busy_time;
			}
			else
			{//recover
				t = Degraded_read(pblk_num, cp);
				Check_F_Cache(cp, pblk_num);
			}
...
//			printf("Access [blk: %5lld] in Node[%2d]: Failure!\n", pblk_num, CurNode);
			long long t = Degraded_read(pblk_num, policy);
...
long long Degraded_read(long long pblk_num, int cp)
{
	...
	for(int i = 0; i < stripe_tbl[stripe_num].data.size() ; i++)
	{
		...
		if(stripe_tbl[stripe_num].data[i] > pblk_num)
			greedy_set.insert(stripe_tbl[stripe_num].data[i]);
	}
	...
	printf("Helper[Node] of Pblk:[%lld]:  ", pblk_num);
	for(int i = 0; i < tmp.size() ; i++)
	{
...
//			printf("ACCESS Node[%d]: %5lld --> ", Cur_node, Cluster[Cur_node]->Last_Busy_Time);
			...
			{
				Cluster[Cur_node]->queques[queue] = sys_last_busy_time + 10*(READ_LATENCY + DATA_TRANSMISSION);
			}
			else
			{
			//	printf("Blocked[Node Busy]!\n");
				read_blk_num++;
				Cluster[Cur_node]->queques[queue] += 10*(READ_LATENCY + DATA_TRANSMISSION);
			}
...
//			printf("%5lld\n", Cluster[Cur_node]->Last_Busy_Time);
		...
		{
			printf("Helper Pblk[%lld] is corrupted\n", tmp[i]);

			if(Access_F_Cache(cp, tmp[i]))//F_cache hit
			{
				F_cache_hit++;
			...
			{
				last_finish_time = Degraded_read(tmp[i], cp);
			}
		}
	...
	//recover the pblk
	if(global_storage[pblk_num]->corrupted == 1)
	{
		global_storage[pblk_num]->corrupted = 0;
	}


	if(cp == 3)
	{
		printf("Greedy Recovery:  ");

		for(si = greedy_set.begin(); si != greedy_set.end(); si++)
		{
			if(global_storage[*si]->corrupted == 1)
			{
				global_storage[*si]->corrupted = 0;
				Check_F_Cache(cp, *si);
				printf(" %5lld [%2lld] ", *si, *si % ec_node);
			}
...
//			printf("[In Request]: Write_index[%lld]\n", CurReq.W_Req_Index);
		...
		else//read
		{
			...
			if(Count_RIO -1 >= 0)
			{
				R_Req_Tbl[policy][Count_RIO-1].G_value = 1 - ((float)R_Req_Tbl[policy][Count_RIO-1].Lasting_time / R_Req_Tbl[policy][Count_RIO-1].datablks.size() / R_Req_Tbl[policy][Count_RIO-1].straggler);
...
//			printf("R_Req_Tbl[%d][%lld]: RID %s\n", policy, Count_RIO-1, R_Req_Tbl[policy][Count_RIO-1].RequestID);
...
void Print_Req_Tbl(std::vector< vector<struct Request> >&Req_Tbl, int write, int policy)
{
	if(write == 1)
	{
...
//	printf("RequestID\tType\tLasting_time\tArrive_time\tFinish_time\n");
	printf("%8s %8s %8s %8s %20s %8s", "RID", "Type", "L_time", "A_Time", "Datablk_set(RA)", "G-value");
	std::cout << "RID\tType\t \t \t Datablk_set(BA) \t  \t Datablk_set(DA)\n";
...
//			}
			...
			printf("%8s %8s %8lld %8lld", Req_Tbl[i][w].RequestID, Req_Tbl[i][w].Type, Req_Tbl[i][w].Lasting_time, Req_Tbl[i][w].Arrive_time);
...
struct Result Process(char **files, int trace_start, int trace_end, struct traceline *T_line, int ec, int policy, int iteratimes, int degraded, int cache_policy)
{
	FILE *file = NULL;
	struct Result res;
	...
	{
		case 0:
			res.cache_name = (char *)"No F cache";
			break;
		case 1:
			res.cache_name = (char *)"LRU_No G or C";
			break;
	    case 2:
	    	res.cache_name = (char *)"ARC_No G or C";
	    	break;
	    case 3:
	    	res.cache_name = (char *)"CoARC";
	    	break;
	    case 4:
	    	res.cache_name = (char *)"FCache";
	    	break;
	    case 5:
	    	res.cache_name = (char *)"DAF ";
	        break;
	    default:
	    	res.cache_name = (char *)"Wrong Failure Policy";
	    	exit(0);
	}
...
//						printf("----------------------------Start Read Process!------------------------\n");
						//Recover_Cluster();
						//Create_Node_Failure(total_line);
						//Cluster[5]->failed = 1;
					}
					...
					read_count++;
					Read_by_FP(T_line, policy, cache_policy);
...
//	Print_stripe_tbl();
//	Print_Blk_Mapping();
	Print_Req_Tbl(R_Req_Tbl, READ, policy);
	Generalize_ReadTrace_ALL(Read_trace, R_Req_Tbl[policy], READ);

//	//find average degraded read performance
	if(degraded == 1)
	{
		float sum = 0;
		float Ave_latency = 0;
		for(int seed = 0; seed < ec_node; seed++)
		{
			Recover_Cluster();
//			Create_Node_Failure(seed);
			Cluster[seed]->failed = 1;
		...
		Ave_latency = sum / ec_node;
		res.total_latency = Ave_latency;
	}
	else
	{
...
//	    printf("total_fp: %lld || totalline: %lld || effectiveline: %lld || writecount: %lld || readcount: %lld || readline_not_by_fp: %lld degraded_read: %lld || othercount: %lld\n", unique_fp, total_line, effective_line_count, write_count, read_count, readline_not_by_fp, degraded_read, other_count);
//
...
////Output_Result(policy);
	printf("failed data %lld, ratio(failed/read): %5.3f\n", globle_failed_blks_count, (float) globle_failed_blks_count / read_count);

	Print_map(globle_failed_blks);
	return res;
