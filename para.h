...
int trace = 1;
int Policy = 1;
int C_Policy = 1;//default is no cache.
int file_exist = -1;
...
std::map<long long, struct fp_node *> Address_Map;
...
std::map<struct fp_node *, Sset> R_Map;
struct Node **Cluster;
...
long long end_recon_point = 0; //it is the ending plk of the recovering stripe.
...
std::set<int>nodeset;
std::map<long long, int>globle_failed_blks;
long long globle_failed_blks_count = 0;
...
//lists for ARC
std::list< struct fp_node *> t1(0);
std::list< struct fp_node *> t2(0);
std::list< struct fp_node *> b1(0);
std::list< struct fp_node *> b2(0);
