...
//just check, not adjustment
int cache_lru_check(cache_lru *lru, long long addr) {
...
//arc check without ajustment
int cache_arc_check(cache_arc *arc, long long addr) {
...
//cache check: check without ajustment
int cache_check(cache *c, long long addr) {
  c->requests++;
  int res = 0;

  switch (c->t) {
    case LRU:
      res = cache_lru_check(c->data.lru, addr);
      break;
    case ARC:
      res = cache_arc_check(c->data.arc, addr);
      break;
  }
...
void Init_F_cache(int i)
{
	...
	{
		printf("[Init]---------LRF without check or greedy!\n");//Normal LRF without greedy
		lru = cache_create(LRU, CACHE_SIZE);
	}
	else if(i == 2)
	{
		printf("[Init]---------ARC without check or greedy!\n");//Normal FC without greedy
