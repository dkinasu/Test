...
void Finish_time_SOE(int sp, int policy)
{
	...
	for (int i = 0; i < ec_m; ++i)
	{
		...
		{
			Cluster[temp]->queques[queue] = sys_last_busy_time + DATA_TRANSMISSION + WRITE_LATENCY;
		}
		else
		{
			blk_in_stripe++;
			// printf("Blocking!!!![%d]\n", blk_in_stripe);
			Cluster[temp]->queques[queue] += WRITE_LATENCY + DATA_TRANSMISSION;
		}
	}
	...
	for (int i = 0; i < soe.data_pblk.size(); ++i)
	{
		...
		if (sys_last_busy_time >= Cluster[temp]->queques[queue])
		{
			Cluster[temp]->queques[queue] = sys_last_busy_time + WRITE_LATENCY + DATA_TRANSMISSION;
		}
		else
		{
			blk_in_stripe++;
...
bool fpcmp (struct traceline &i,struct traceline &j) { return (i.fp->replica<j.fp->replica); }

void Output_Relationship(char * file_prefix)
{
	char *result_prefix = (char *)".R";

	char *trace = (char *)malloc(20 * sizeof(char));

	char *R_output = (char *)malloc(500 * sizeof(char));

	sprintf(trace, "%s%d%s%d", file_prefix, trace_start, "-", trace_end);

	sprintf(R_output, "%s%s", trace, result_prefix);

	long long not_equal = 0;
	...
	long long fp_count = HASH_COUNT(fp_store);
//	printf("fp_count= %lld\n", fp_count);
//	Print_fps();

	HASH_ITER(hh, fp_store, current_user, tmp)
	{
		if(current_user->replica != 0)
		{
			replicated++;
//			sprintf(result_buf, "%s %5d %5ld %5d\n", current_user->fingerprint, current_user->replica, R_Map[current_user].size() -1, current_user->access_times);
//			result_buf[strlen(result_buf)] = '\0';
//			fwrite(result_buf, 1, strlen(result_buf), R_out);

			if(current_user->replica != R_Map[current_user].size() -1)
			{
				not_equal++;
			}
		}
	...
	sprintf(result_buf, "totalline: %lld || effectiveline: %lld || writecount: %lld || readcount: %lld || readline_not_by_fp: %lld degraded_read: %lld || othercount: %lld\n", total_line, effective_line_count, write_count, read_count, readline_not_by_fp, degraded_read, other_count);
	result_buf[strlen(result_buf)] = '\0';
	fwrite(result_buf, 1, strlen(result_buf), R_out);

	sprintf(result_buf, "Dup_rate: %f  R/W [%4.3f] \n", (double)(write_count-fp_count)/(double)write_count, (float) read_count / write_count);
	result_buf[strlen(result_buf)] = '\0';
	fwrite(result_buf, 1, strlen(result_buf), R_out);


	sprintf(result_buf, "[Unequal ratio(not_equal/replicated) is: %3f] total_fp:%lld replicated:%lld not_equal:%lld\n", (float)not_equal/replicated, fp_count, replicated, not_equal);
	result_buf[strlen(result_buf)] = '\0';
	fwrite(result_buf, 1, strlen(result_buf), R_out);
...
//			global_storage[temp[i]]->dummy = 1;
		...
		else
		{
			...
			Cluster[soe.data_pblk[i].fp->p->node_num]->dup_count += soe.data_pblk[i].fp->replica;
			Cluster[soe.data_pblk[i].fp->p->node_num]->pblk_used++;
			Cluster[soe.data_pblk[i].fp->p->node_num]->write_access++;
			Cluster[soe.data_pblk[i].fp->p->node_num]->speclative_read_access += soe.data_pblk[i].fp->replica + 1;
...
void Data_corruption(long long blk_num)
{
	int flag = -1;
	srand(time(NULL) + blk_num);

	for(int i = -1; i < global_storage[blk_num]->count_corrupted; i++)
	{
		flag = rand() % 10 + 1;
...
//		printf("i= %d flag=%d\n", i, flag);
		if(flag %5 == 0)
		{
			global_storage[blk_num]->corrupted = 1;
			global_storage[blk_num]->count_corrupted++;
			globle_failed_blks[blk_num]++;
			globle_failed_blks_count++;
//			printf("Pblk[%lld]: corrupted!\n", blk_num);
			break;
...
//	global_storage[blk_num]->corrupted = 1;
//	global_storage[blk_num]->count_corrupted++;
//	printf("Degraded Pblk[%lld]: corrupted!\n", blk_num);
...
void Reset_all(int node_num, int cache_size_N)
{
	...
	readline_not_by_fp = 0;
	datablk_serial = 0;
	last_datablk_by_fp = -1;
	//stripe
	Reset_stripe_tbl();
