...
struct Result Process(char **files, int trace_start, int trace_end, struct traceline *T_line, int baseline, int policy, int iteratimes, int degraded, int cache_policy);
void Write_Process(struct traceline *T_line, int baseline, int policy);
void Read_by_FP(struct traceline *T_line, int baseline, int cp);
...
int Request(struct traceline *T_line, int ec);
void Print_Req_Tbl(std::vector<struct Request> &Req_Tbl, int write, int policy);
