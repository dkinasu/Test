...
//			printf("Request[%d]: %lld\n", i, ReadTrace[i].Lasting_time);
		}

	}

	ave_L = total_read_time / ReadTrace.size();

	t.total_latency = ave_L;
}
...
void Input_Result(struct Result &t, int c)
{
	...
	{
		t.hits = cache->hits;
		t.requests = cache->requests;
		t.adjusts = cache->adjusts;
...
//			printf(" %2lld ", ReadTrace[i].datablks[j] % ec_node);
			printf(" %2lld ", ReadTrace[i].datablks[j]);
		}
...
{  
	...
	if (argc < 6) {
		...
		printf("Usage: ./main [tracefolder_path/] [(policy)1|2] [start_num] [end_num] [node_num] [cache_policy] [Degraded] [cache_size]\n");
		return -1;
	}
...
//    (*p)[1] = (char *)"./trace/fiu/homes/";//CAFTL trace
    ...
    (*p)[2] = (char *)"2";
    (*p)[3] = (char *)"3";
    (*p)[4] = (char *)"12"; //Node_num
    (*p)[5] = (char *)"5"; //C_policy
    (*p)[6] = (char *)"0"; //degraded
    (*p)[7] = (char *)"0"; //Cache_size
